...
function ApplyJob() {
  ...
  const [isAlreadyApplied , setIsAplreadyApplied] = useState(false)

  const { jobs, backendurl , userData , userApplications, fetchUserApplications } = useContext(AppContex)
  ...
  const checkAlreadyApplied = () => {
    ...
    setIsAplreadyApplied(hasApplied)
              ...
              <h2>More Jobs from {jobData.companyId.name}</h2>
              {
                jobs.filter(job => job._id !== jobData._id && job.companyId._id === jobData.companyId._id)
                  .filter(job => {
                    ...
                    //return user has not already applied for this job
