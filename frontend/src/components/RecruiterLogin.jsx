...
const RecruiterLogin = () => {
  ...
  const {setShowRecruiterLogin} = useContext(AppContex)
  ...
  return (
    <div className='absolute top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center '>
      <form onSubmit={onSubmitHandler} className='relative bg-white p-10 rounded-xl text-slate-500'>
        ...
        {
          state === "Login" ? <p className='mt-5 text-center'>Don't have an account?  <span className='text-green-500 cursor-pointer' onClick={() => setState("Sign Up")}>Sign up</span></p> :
            <p className='mt-5 text-center'>Already have an account? <span className='text-green-500 cursor-pointer' onClick={() => setState("Login")}>Login</span></p>
        }
